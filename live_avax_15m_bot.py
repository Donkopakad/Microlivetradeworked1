...
def build_exchange(config: Config) -> ccxt.binance:
    ...
    if not config.dry_run:
        ...
        safe_call(lambda: exchange.set_leverage(config.leverage, SYMBOL), f"set {config.leverage}x leverage")
...
def find_signal(
    candles: list[Candle],
    last_trade_close_candle_time: int | None,
) -> tuple[str | None, list[Candle]]:
    eligible = [
        c for c in candles
        if last_trade_close_candle_time is None or c.close_time_ms > last_trade_close_candle_time
    ]
    ...
    # Extra candle detail for debugging chart mismatch
    for c in last_five:
        log(
            f"CANDLE {utc_to_ist_str(c.open_time_ms)} "
            f"open={c.open} close={c.close} color={c.color}"
        )
...
def ensure_trade_csv(path: Path) -> None:
    ...
    with path.open("w", newline="") as fh:
        ...
        writer.writerow([
            ...
            "pnl_pct",
            "dry_run",
            "entry_candle_open_ist",
            "exit_candle_close_ist",
        ])
...
def append_trade(path: Path, trade: ActiveTrade, exit_price: float, exit_ms: int, dry_run: bool) -> None:
    ...
    with path.open("a", newline="") as fh:
        csv.writer(fh).writerow([
            utc_to_ist_str(trade.opened_at_ms),
            utc_to_ist_str(exit_ms),
            SYMBOL,
            trade.side,
            f"{trade.quantity:.8f}",
            ...
            f"{pnl_pct:.4f}",
            dry_run,
            utc_to_ist_str(trade.entry_candle_open_ms),
            utc_to_ist_str(trade.entry_candle_open_ms + TIMEFRAME_MS),
        ])
...
def open_trade(exchange: ccxt.binance, config: Config, side: str, candle_start_ms: int) -> ActiveTrade:
    ...
    log(
        f"Signal detected: {side.upper()} "
        f"entry_price={entry_price:.6f} quantity={quantity:.8f} dry_run={config.dry_run}"
    )
    ...
    if not config.dry_run:
        ...
        order = safe_call(
            lambda: exchange.create_market_order(SYMBOL, order_side, quantity),
            f"open {side} market order",
        )
    ...
    return ActiveTrade(
        side=side,
        entry_candle_open_ms=candle_start_ms,
        entry_price=entry_price,
        quantity=quantity,
        opened_at_ms=fetch_exchange_ms(exchange),
        order_id=order_id,
    )
...
def close_trade(exchange: ccxt.binance, config: Config, trade: ActiveTrade) -> tuple[float, int]:
    ...
    if not config.dry_run:
        ...
        order = safe_call(
            ...
            f"close {trade.side} market order",
        )
...
def sleep_until_next_candle(exchange: ccxt.binance, poll_seconds: int) -> int:
    """Sleep until the next 15m candle starts.

    Important:
    If Binance server time temporarily fails due to DNS/internet issue,
    this function will NOT crash the bot. It waits and retries.
    """
    while running:
        try:
            server_ms = fetch_exchange_ms(exchange)
        except Exception as e:
            log(f"Binance server time temporarily unavailable; retrying in 10 seconds: {e}")
            time.sleep(10)
            continue

        current_start = server_ms - (server_ms % TIMEFRAME_MS)
        next_start = current_start + TIMEFRAME_MS
        ...
        if wait_ms <= 1500:
            ...
            try:
                ...
                log(f"Could not confirm current candle start; retrying in 10 seconds: {e}")
                time.sleep(10)
                continue
    ...
    try:
        ...
        return int(time.time() * 1000) - (int(time.time() * 1000) % TIMEFRAME_MS)
...
def main() -> int:
    ...
    log(
        f"Starting AVAX/USDT 15m Binance Futures bot "
        f"dry_run={config.dry_run} size={config.trade_usdt_size} USDT "
